...
*** Enumerate samples
 ...
 #+BEGIN_SRC sh
   ...
   (
       cd samples
       pwd
       ../../R/sample-words.R -n 5 -w 75 --minlen 2 --maxlen 6  ../../words/liste.de.mots.francais.frgut.txt
   )
 #+END_SRC
...
#+BEGIN_SRC sh
  OUTDIR=results-classified
  TEMPFILE=`mktemp`
  wc -l $OUTDIR/* | while read LINECOUNT DBFILE ; do
...
#+BEGIN_SRC R
  ...
  dbfiles <- list.files("../cryptarithms/", full.names=TRUE)
