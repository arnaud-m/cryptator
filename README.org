...
** Prerequisites
  ...
  - Cryptarithmetic Puzzle Solver in C (optional)
    ...
    - If compilation fails with recent GCC versions, compile with ~-std=gnu89~ to preserve compatibility with the old C syntax : ~make CFLAGS="-std=gnu89 -g -O~.
...
** As a stand-alone application
   ...
   There are two console applications and the links below point to demonstration scripts.
